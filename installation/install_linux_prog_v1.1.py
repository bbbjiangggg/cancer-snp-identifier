...
subprocess.run(['wget', 'https://ftp-trace.ncbi.nlm.nih.gov/sra/sdk/current/sratoolkit.current-ubuntu64.tar.gz'])
subprocess.run(['tar', '-xvzf', 'sratoolkit.current-ubuntu64.tar.gz'])
subprocess.run(['rm', 'sratoolkit.current-ubuntu64.tar.gz'])
subprocess.run(['mv', 'sratoolkit.2.10.9-ubuntu64', 'sratoolkit'])
subprocess.run(['export', 'PATH=$PATH:~/sratoolkit/bin'], shell=True)
...
subprocess.run(['rm', 'Trimmomatic-0.39.zip'])
subprocess.run(['sudo', 'cp', 'Trimmomatic-0.39/trimmomatic-0.39.jar', '$HOME/local/bin'])
subprocess.run(['sudo', 'chmod', 'a+x', '$HOME/local/bin/trimmomatic-0.39.jar'])
print(f'{MAGENTA}Checking if Trimmomatic was properly installed{RESET}')
subprocess.run(['java', '-jar', '$HOME/local/bin/trimmomatic-0.39.jar'])
