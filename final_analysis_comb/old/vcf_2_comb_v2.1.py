...
def compress_vcf_files(directory):
    for file in directory.iterdir():
        if file.suffix == ".vcf":
            ...
            try:
                subprocess.run(["bgzip", str(file)], check=True)
...
def download_normal_vcf(link_list, output_directory, chr, cancer):
    log_message("Would you like to download a normal combined VCF file (yes or no)? ", "blue")
    normal_file = input().strip().lower()
    
    if normal_file == 'yes':
        log_message("Enter the chromosome number of interest (1-22, X or Y): ", "blue")
...
def main():
    ...
    if vcf_files:
        ...
        combined_vcf_file = parent_directory / f"ch{chr}_{cancer}_comb.vcf"
        ...
        download_normal_vcf(link_list, parent_directory, chr, cancer)

        # Move the generated files into the output directory
        files_to_move = [
            f"ch{chr}_normal_comb.vcf",
            f"ch{chr}_{cancer}_comb.vcf",
            f"ch{chr}_{cancer}_comb.vcf.gz",
            f"ch{chr}_{cancer}_comb.vcf.gz.tbi"
        ]

        move_files_to_directory(parent_directory, chr, cancer, files_to_move)
    else:
