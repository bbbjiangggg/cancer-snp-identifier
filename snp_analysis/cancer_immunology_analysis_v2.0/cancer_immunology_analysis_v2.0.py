    ...
    for accession_number in accession_numbers_to_analyze:
        ...
        if (read_type == '1' and os.path.isfile(trimmed_file_single)) or \
           (read_type == '2' and os.path.isfile(trimmed_file_paired_1) and os.path.isfile(trimmed_file_paired_2)):
            ...
            # Attempt to download the data using fasterq-dump
            try:
                if read_type == '1' and not os.path.isfile(trimmed_file_single):
                    log_message(f"\nDownloading single-end sequence number {accession_number} with fasterq-dump...", level="info")
                    ...
                    log_message(f"\nDownloading paired-end sequence number {accession_number} with fasterq-dump...", level="info")
                    run_command(f"fasterq-dump {accession_number} --split-files --threads {threads} --progress")
                ...
                log_message(f"An error occurred while processing {accession_number} with fasterq-dump. Please check the log for details.", level="error")
                sys.exit(1)
