...
from snp_analysis_pipeline_v3.data_processing import detect_read_type, download_and_convert, trim_reads
...
def print_banner():
    banner_lines = [
        "🧬🦠🔬 CANCER IMMUNOLOGY 🔬🦠🧬",
        "🔥🛠️ SNP Analysis Pipeline v3 🛠️🔥",
        "⚡💡 Precision Oncology Research 💡⚡",
        "🔵==============================🔵"
    ]
    
    for line in banner_lines:
        print(colored(line.center(50), "cyan", attrs=["bold"]))
...
def main():
    ...
    chromosomes_list = [str(i) for i in range(1, 23)] + ['X', 'Y'] if chromosomes_input.lower() == 'all' else [c.strip().upper() for c in chromosomes_input.split(',')]
