...
# Asking the user if the analysis is single or paired-end
...
# Use the appropriate bash script based on the user's choice
...
mkdir SRR_one
mv SRR_one.fastq SRR_one/SRR_one.fastq

echo -e "\n\033[1;35mRunning fastqc on SRR_one...\033[0m "
fastqc SRR_one/SRR_one.fastq

echo -e "\n\033[1;35mTrimming SRR_one...\033[0m "
java -jar trim_path SE -threads 4 SRR_one/SRR_one.fastq SRR_one/SRR_one_trimmed.fq.gz ILLUMINACLIP:truseq3_path:2:30:10 SLIDINGWINDOW:4:20 MINLEN:35
...
samtools sort -o SRR_one/SRR_one_mapped.bam > SRR_one/SRR_one_mapped.sorted.bam
...
echo -e "\n\033[1;35mDownloading number sequence SRR_one from SRA...\033[0m "
fastq-dump --split-files SRR_one
...
mkdir SRR_one
mv SRR_one_1.fastq SRR_one/SRR_one_1.fastq
mv SRR_one_2.fastq SRR_one/SRR_one_2.fastq

echo -e "\n\033[1;35mRunning fastqc on SRR_one...\033[0m "
fastqc SRR_one/SRR_one_1.fastq
fastqc SRR_one/SRR_one_2.fastq

echo -e "\n\033[1;35mTrimming SRR_one...\033[0m "
java -jar trim_path PE -threads 4 SRR_one/SRR_one_1.fastq SRR_one/SRR_one_2.fastq SRR_one/SRR_one_trimmed_1.fq.gz SRR_one/SRR_one_unpaired_1.fq.gz SRR_one/SRR_one_trimmed_2.fq.gz SRR_one/SRR_one_unpaired_2.fq.gz ILLUMINACLIP:truseq3_path:2:30:10 SLIDINGWINDOW:4:20 MINLEN:35

echo -e "\n\033[1;35mRunning fastqc on trimmed SRR_one...\033[0m "
fastqc SRR_one/SRR_one_trimmed_1.fq.gz
fastqc SRR_one/SRR_one_trimmed_2.fq.gz

echo -e "\n\033[1;35mMapping reads using Bowtie2...\033[0m "
bowtie2 --very-fast-local -x bowtie2_path -1 SRR_one/SRR_one_trimmed_1.fq.gz -2 SRR_one/SRR_one_trimmed_2.fq.gz -S SRR_one/SRR_one_mapped.sam
...
rm SRR_one/SRR_one.fastq SRR_one/SRR_one_mapped.sam SRR_one/SRR_one_mapped.bam SRR_one/SRR_one_mapped.raw.bcf SRR_one/SRR_one_fastqc.zip SRR_one/SRR_one_trimmed_fastqc.zip
...
ref_genome_path = input(f'{MAGENTA}6){RESET} Copy and paste the complete path to your BWA reference genome: ')
replace_in_untrimmed_bash_srr('ref_genome_path', ref_genome_path)
...
for d in all_dirs:
    ...
    vcf_file = os.path.join(search_dir, d, f'{d}_mapped.sorted.bam')
...
for sra in srr_list:
    ...
    sra_dir = f'{cwd}/{sra}'
    vcf_file = f'{sra_dir}/{sra}_mapped.sorted.bam'
...
def run_analysis():
    ...
    while True:
        ...
        if choice.lower() == 'yes':
            ...
            print(f'{MAGENTA} 5) {RESET} BWA reference chromosome path: ', ref_genome_path)
            print(f'{MAGENTA} 6) {RESET} Accession list file name: ', accession)
            os.system('python3 untrimmed_bam_tool_v1.0.py')
...
# Execute the correct analysis based on user input
if analysis_type == "single":
    # Call the function or logic for single-end analysis
    pass
else:
    # Call the function or logic for paired-end analysis
    pass
