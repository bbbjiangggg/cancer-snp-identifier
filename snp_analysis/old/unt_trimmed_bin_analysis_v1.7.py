...
def main():
    ...
    fastp_path = "/usr/local/bin/fastp"
    ...
    if chromosomes_input.lower() == 'all':
        vcf_option = input(colored("Would you like a combined VCF file or separated VCF files for the chromosome results? (type 'combined' or 'separated'): ", "magenta")).strip().lower()
    ...
    for accession_number in accession_numbers_to_analyze:
        ...
        if not os.path.isfile(trimmed_file):
            print(colored(f"\nDownloading sequence number {accession_number} from SRA...", "magenta"))
            ...
            print(colored(f"\nRunning fastqc on {accession_number}...", "magenta"))
            run_command(f"fastqc {accession_number}/{accession_number}.fastq")

            print(colored(f"\nTrimming {accession_number} with fastp...", "magenta"))
            trim_command = f"{fastp_path} -i {accession_number}/{accession_number}.fastq -o {trimmed_file} --thread=4 --html={accession_number}/{accession_number}_fastp_report.html --json={accession_number}/{accession_number}_fastp_report.json"
            run_command(trim_command)

            print(colored(f"\nRunning fastqc on trimmed {accession_number}...", "magenta"))
            run_command(f"fastqc {trimmed_file}")
        else:
            print(colored("Trimmed file already exists. Skipping download, trimming, and quality check...", "magenta"))
        ...
        for chromosome in chromosomes_list:
            ...
            if os.path.isfile(final_vcf_file) and not is_file_empty(final_vcf_file):
                print(colored(f"VCF file for {accession_number}, chromosome {chromosome} already exists. Skipping analysis...", "magenta"))
                continue
            elif is_file_empty(final_vcf_file):
                print(colored(f"VCF file for {accession_number}, chromosome {chromosome} is empty. Deleting and adding to analysis...", "magenta"))
            ...
            print(colored(f"\nMapping {accession_number} reads using Bowtie2 for chromosome {chromosome}...", "magenta"))
