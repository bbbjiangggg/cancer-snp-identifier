...
print(f'{MAGENTA}ONLY RUN THIS PROGRAM IN YOUR HOME DIRECTORY{RESET}')
print(f'{MAGENTA}To access your home directory, open Finder and then click on Go and Home.{RESET}')
input(f'{MAGENTA}Press enter to continue...{RESET}\n')

# Checking for the installation of Xcode
print(f'{MAGENTA}Checking for the installation of Xcode{RESET}')
result = subprocess.run(['xcode-select', '-p'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
if 'error' in result.stderr.decode('utf-8'):
    print(f'{RED}Xcode is not installed{RESET}')
    # Install Xcode command line tools
    print(f'{MAGENTA}Installing Xcode command line tools{RESET}')
    subprocess.run(['xcode-select', '--install'])
    input(f'{MAGENTA}Press enter to continue...{RESET}\n')
else:
    print(f'{GREEN}Xcode is installed{RESET}')
    input(f'{MAGENTA}Press enter to continue...{RESET}\n')

# Checking for installation of Homebrew
print(f'{MAGENTA}Checking for installation of Homebrew{RESET}')
...
if 'command not found' in result.stderr.decode('utf-8'):
    ...
    # Install Homebrew
    print(f'{MAGENTA}Installing Homebrew{RESET}')
